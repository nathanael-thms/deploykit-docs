...
- `PRE_FLIGHT_CHECKS`:  A `true`/`false` variable relevant only for both deployment types. When `true`, the script automatically performs pre flight checks as described in [pre-flight checks](deployment.md#pre-flight-checks)

- `MIN_STORAGE_GB`: Specifies the npm command to run. This can be omitted if `PRE_FLIGHT_CHECKS="false"`; if present but `PRE_FLIGHT_CHECKS="false"`, it will be ignored. Specifies how much storage to check for in pre flight checks as described in [pre-flight checks](deployment.md#pre-flight-checks)
