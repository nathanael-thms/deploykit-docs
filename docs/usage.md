...
| `migrate` | Same as selecting option 2. Starts migration to symlink deployment as described in [Migration to symlink deployment](utilities.md#migrate-to-symlink-deployment). Requires user interaction. |
| `revert` | Same as selecting option 3 (symlink only). Reverts to a previous deployment as described in [Reverting to a previous deployment](utilities.md#revert-to-a-previous-deployment). Requires user interaction. |
...
| `cleanup` | Same as selecting option 5 (symlink only). Cleans up old releases as described in [Cleaning up old releases](utilities.md#clean-up-old-releases). Can be followed by `-<n>` (for example `--cleanup-10`) to keep the latest `n` releases. Requires user interaction if not followed by an integer. |
...
This starts the migration to symlink deployment as described in [Migration to symlink deployment](utilities.md#migrate-to-symlink-deployment). It calls `utilities/migrate_to_symlink.sh`.
...
Cleans up old releases when using symlink deployment, as described in [Cleaning up old releases](utilities.md#clean-up-old-releases). It calls `utilities/clean_up_releases.sh`.
